-- ============================================================
-- 018_fix_bypass_set_local.sql — BYPASS VIA SET LOCAL (TESTE)
-- ------------------------------------------------------------
-- As migrations 015/017 usaram set_config('app.bypass_estat', ..., true/false)
-- e o gatilho não enxergava o valor no contexto do PostgREST (o UPDATE
-- disparava "Alteração de campos restritos"). Esta migration troca para a
-- forma SQL nativa SET LOCAL (que define o GUC para a transação corrente de
-- forma garantida, antes de qualquer statement), aplicada no INÍCIO das
-- funções de pontuação.
-- NÃO-DESTRUTIVO: reescreve as duas funções de pontuação.
-- ============================================================

-- ------------------------------------------------------------
-- 1. registrar_desafio_no_ledger — SET LOCAL no início
-- ------------------------------------------------------------
...
BEGIN
  -- Define o GUC de bypass para a TRANSAÇÃO corrente (antes de qualquer
  -- statement que dispare o gatilho). O gatilho bloquear_autopromocao lê
  -- current_setting('app.bypass_estat', true) e libera apenas estatisticas.
  ...
  -- Desliga o GUC de bypass ao final (sessão limpa para o próximo request).
...
-- ------------------------------------------------------------
-- 2. pontuar_quiz — SET LOCAL no início
-- ------------------------------------------------------------
...
DECLARE
  ...
  v_config JSONB;
BEGIN
  -- GUC de bypass para a transação corrente (liberado só para o UPDATE de
  -- estatisticas; o gatilho valida que os demais campos não mudam).
  PERFORM set_config('app.bypass_estat', 'on', true);
