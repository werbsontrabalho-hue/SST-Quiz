-- ============================================================
-- 021_pontuar_com_tabela_real.sql — RPCs USAM A TABELA DE AUTORIZAÇÃO
-- ------------------------------------------------------------
-- Aplica o mecanismo da 020 (tabela real autorizacoes_pontuacao) nas
-- funções de pontuação: antes de gravar estatisticas, o RPC autoriza o
-- usuário; após gravar, revoga. O gatilho V-018 aceita o UPDATE apenas com
-- autorização válida.
...
BEGIN
  ...
  IF v_colaborador_id IS NOT NULL THEN
    -- Autoriza o UPDATE de estatisticas deste usuário (tabela de controle).
    ...
    -- Revoga a autorização após gravar.
    PERFORM public.revogar_pontuacao(v_colaborador_id);
...
BEGIN
  ...
  IF p_vencedor_id IS NOT NULL THEN
    ...
    PERFORM public.autorizar_pontuacao(p_vencedor_id);
    ...
    PERFORM public.revogar_pontuacao(p_vencedor_id);
  END IF;
  ...
  IF p_perdedor_id IS NOT NULL AND p_perdedor_id <> p_vencedor_id THEN
    ...
    PERFORM public.autorizar_pontuacao(p_perdedor_id);
    ...
    PERFORM public.revogar_pontuacao(p_perdedor_id);
  END IF;
