...
import { saveOrShareFile } from '../../../lib/capacitor';
...
export const ProvaAvaliacaoPDFModal: React.FC<ProvaAvaliacaoPDFModalProps> = ({
  ...
  // Baixar arquivo PDF (Web: download direto; Capacitor Android/iOS: Share sheet nativo)
  const handleBaixarPdfDireto = async () => {
    setGerandoPdf(true);
    try {
      const resData = await gerarPdfBlob(false);
      if (resData) {
        await saveOrShareFile(resData.blob, resData.filename, 'application/pdf');
      } else {
  ...
  const handleEnviarEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiFetch('/api/enviar_email_prova', {
        method: 'POST',
        headers,
        body: JSON.stringify({
