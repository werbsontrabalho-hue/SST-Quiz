...
export const triggerDownloadCSV = (filename: string, csvContent: string) => {
  ...
  // Capacitor (Android/iOS): Share sheet nativo; Web: <a download> (comportamento preservado).
  // Fire-and-forget para manter a assinatura síncrona usada pelas telas.
  void import('../lib/capacitor').then(({ saveOrShareFile }) =>
    saveOrShareFile(blob, filename, 'text/csv;charset=utf-8')
  ).catch(() => {
